...
# solution(l) that returns the same list sorted in ascending order by major,
...
def solution(l):
    ...
    versions.sort(key=lambda x: (int(x[0]), int(x[1]) if len(x) > 1 else -1, int(x[2]) if len(x) > 2 else -1, len(x)))
...
def solution(l):
    ...
    versions.sort(key=lambda x: (int(x[0]), int(x[1]) if len(x) > 1 else 0, int(x[2]) if len(x) > 2 else 0))

    for i, version in enumerate(versions):
        versions[i] = '.'.join(version)
    print(versions)
