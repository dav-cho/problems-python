...
#   number of atoms in the universe. However, the lazy programmers (who made a
#   big news by inventing the Y2K bug and other related things) apparently
#   didn't think thoroughly enough about this, and so their simple date-time
#   system can only record time from 0 to t-1 seconds. If the elder received
#   the total amount of time (in seconds) more than the system can store, it
#   will be wrapped around so that the time would be between the range 0 to t-1.
...
# Because the system can only store time between 0 to 99 seconds, the first
# 100 seconds of time will be lost, giving the answer of 5.
...
# but naive solutions will time out easily. Good luck, and do not displease
# the Elder.
...
class Test(unittest.TestCase):
    def test_cases(self):
        ...
        #self.assertEqual(elder_age(8,8,0,100007), 224)
        #self.assertEqual(elder_age(25,31,0,100007), 11925)
        #self.assertEqual(elder_age(5,45,3,1000007), 4323)
        #self.assertEqual(elder_age(31,39,7,2345), 1586)
        #self.assertEqual(elder_age(545,435,342,1000007), 808451)
        # You need to run this test very quickly before attempting the actual tests :)
        #self.assertEqual(elder_age(28827050410, 35165045587, 7109602, 13719506), 5456283)
