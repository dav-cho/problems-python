...
############################################################
# Time: O(nmin(k,n)) - It costs O(min(k,n)) time for each linear search. Note
#                      that we do at most nn comparisons in one search even if
#                      k can be larger than n.
# Space: O(1) - We only used constant auxiliary space.
...
################################################
# Time: O(nlog(min(n,k))) - We iterate through the array of size n. For each
#                           iteration, it costs O(logmin(k,n)) time (search,
#                           insert or delete) in the BST, since the size of the
#                           BST is upper bounded by both k and n.
# Space: O(min(n,k)) - Space is dominated by the size of the BST, which is
#                      upper bounded by both k and n.
...
#######################################
# Time: O(n) - For each of the n elements, we do at most three searches, one
#              insert, and one delete on the HashMap, which costs constant time
#              on average. Thus, the entire algorithm costs O(n) time.
# Space: O(min(n,k)) - Space is dominated by the HashMap, which is linear to
#                      the size of its elements. The size of the HashMap is
#                      upper bounded by both n and k. Thus the space complexity
#                      is O(min(n,k)).
