##
#### Sort Array By Parity (easy)
####################################

# Given an array nums of non-negative integers, return an array
# consisting of all the even elements of nums, followed by all
# the odd elements of nums.

# You may return any answer array that satisfies this condition.

# Example 1:
# Input: nums = [3,1,2,4]
# Output: [2,4,3,1]
# The outputs [4,2,3,1], [2,4,1,3], and [4,2,1,3] would also be accepted.

# Note:
# 1 <= nums.length <= 5000
# 0 <= nums[i] <= 5000

################################################################################


## two pointer - outside in
##################
# time: O(n)
# space: 0(1)
class Solution:
    def sort_array_by_parity(self, nums: list[int]) -> list[int]:
        evens, odds = 0, len(nums) - 1
        
        while evens < odds:
            if nums[evens] % 2 == 0:
                evens += 1
            if nums[evens] % 2 == 1:
                nums[evens], nums[odds] = nums[odds], nums[evens]
                odds -= 1

        return nums


class Solution:
    def sortArrayByParity(self, nums: List[int]) -> List[int]:
        evens, odds = 0, len(nums) - 1
        
        while evens < odds:
            if nums[evens] % 2 > nums[odds] % 2:
                nums[evens], nums[odds] = nums[odds], nums[evens]
                
            if nums[evens] % 2 == 0:
                evens += 1
            if nums[odds] % 2 == 1:
                odds -= 1

        return nums


## Tests
############

solution = Solution()

test1 = [3, 1, 2, 4]
result1 = solution.sort_array_by_parity(test1)
print(result1)
# [2, 4, 3, 1]
# The outputs [4,2,3,1], [2,4,1,3], and [4,2,1,3] would also be accepted.

test2 = [0, 2]
result2 = solution.sort_array_by_parity(test2)
print(result2)
# [0, 2]

test3 = [1, 0]
result3 = solution.sort_array_by_parity(test3)
print(result3)
# [0, 1]

test4 = [1, 3]
result4 = solution.sort_array_by_parity(test4)
print(result4)
# [1, 3]


## LeetCode Solutions
#########################

## Approach 1: Sort
#######################
# time: O(n log(n)) - n is length of A
# space: O(n) - for the sort, depending on the built-in implementation
class Solution(object):
    def sort_array_by_parity(self, A):
        A.sort(key = lambda x: x % 2)
        return A


## Approach 2: Two Pass
###########################
# time: O(n) - where n is length of A
# space: O(n) - the space used by the answer
class Solution(object):
    def sortArrayByParity(self, A):
        return ([x for x in A if x % 2 == 0] +
                [x for x in A if x % 2 == 1])


## Approach 3: In-Place
###########################
# time: O(n) - where n is length of A and we only need one pass
# space: O(1) - no additional space needed
class Solution(object):
    def sortArrayByParity(self, A):
        i, j = 0, len(A) - 1
        while i < j:
            if A[i] % 2 > A[j] % 2:
                A[i], A[j] = A[j], A[i]

            if A[i] % 2 == 0: i += 1
            if A[j] % 2 == 1: j -= 1

        return A


## Tests
############

lc_solution = Solution()

test1 = [3, 1, 2, 4]
result1 = lc_solution.sort_array_by_parity(test1)
print(result1)
# [2, 4, 3, 1]
# The outputs [4,2,3,1], [2,4,1,3], and [4,2,1,3] would also be accepted.

test2 = [0, 2]
result2 = lc_solution.sort_array_by_parity(test2)
print(result2)
# [0, 2]

test3 = [1, 0]
result3 = lc_solution.sort_array_by_parity(test3)
print(result3)
# [0, 1]

test4 = [1, 3]
result4 = lc_solution.sort_array_by_parity(test4)
print(result4)
# [1, 3]

